...
namespace Sydewa
{
    ...
    public class LightingManager : MonoSingleton<LightingManager>
    {
        ...
        //but you can experiment and try it yourself. This is only to prevent the script skipping the event because it didn't exactly get to 6pm or whatever the time of the event was. 
        ...
        private void UpdateLighting(float timePercent)
        {
            ...
            if (SunDirectionalLight != null)
            {
                ...
                {
                    case RotationAxis.X:
                        rotationEuler = new Vector3((timePercent * 360f) - 90f, sunYRotation , SunDirectionalLight.transform.localRotation.z);
                        break;
                    case RotationAxis.Y:
        ...
        private void UpdateMoonLighting(float timePercent)
        {
            ...
            if (IsMoonRotationOn)
            {
                ...
                {
                    case RotationAxis.X:
                        rotationEuler = new Vector3((timePercent * 360f) + 90f, moonYRotation, MoonDirectionalLight.transform.localRotation.z);
                        break;
                    case RotationAxis.Y:
