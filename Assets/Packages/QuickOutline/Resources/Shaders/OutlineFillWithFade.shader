Shader "Custom/Outline Fill with Fade" {
  Properties {
    [Enum(UnityEngine.Rendering.CompareFunction)] _ZTest("ZTest", Float) = 0

    _OutlineColor("Outline Color", Color) = (1, 1, 1, 1)
    _OutlineWidth("Outline Width", Range(0, 10)) = 2
    _StencilRef("Stencil Ref", Float) = 2
    _FadeRange("Fade Range", Float) = 1.0  // Distance over which the fade occurs
  }

  SubShader {
    Tags {
      "Queue" = "Transparent+110"
      "RenderType" = "Transparent"
      "DisableBatching" = "True"
    }

    Pass {
      Name "Fill"
      Cull Off
      ZTest [_ZTest]
      ZWrite Off
      Blend SrcAlpha OneMinusSrcAlpha
      ColorMask RGB

      Stencil {
        Ref [_StencilRef] 
        Comp NotEqual
      }

      CGPROGRAM
      #include "UnityCG.cginc"

      #pragma vertex vert
      #pragma fragment frag

      struct appdata {
        float4 vertex : POSITION;
        float3 normal : NORMAL;
        float3 smoothNormal : TEXCOORD3;
        UNITY_VERTEX_INPUT_INSTANCE_ID
      };

      struct v2f {
        float4 position : SV_POSITION;
        fixed4 color : COLOR;
        float3 worldPos : TEXCOORD1;
        UNITY_VERTEX_OUTPUT_STEREO
      };

      uniform fixed4 _OutlineColor;
      uniform float _OutlineWidth;
      uniform float _GlobalYClip;
      uniform float _FadeRange;

      v2f vert(appdata input) {
        v2f output;

        UNITY_SETUP_INSTANCE_ID(input);
        UNITY_INITIALIZE_VERTEX_OUTPUT_STEREO(output);

        float3 normal = any(input.smoothNormal) ? input.smoothNormal : input.normal;
        float3 viewPosition = UnityObjectToViewPos(input.vertex);
        float3 viewNormal = normalize(mul((float3x3)UNITY_MATRIX_IT_MV, normal));

        output.position = UnityViewToClipPos(viewPosition + viewNormal * -viewPosition.z * _OutlineWidth / 1000.0);
        output.color = _OutlineColor;
        output.worldPos = mul(unity_ObjectToWorld, input.vertex).xyz;  // Calculate world position

        return output;
      }

      fixed4 frag(v2f input) : SV_Target {
        // Calculate fade factor based on Y distance from the clip level
        float fadeFactor = saturate((input.worldPos.y - _GlobalYClip) / _FadeRange);
        
        // Calculate final alpha with fade
        float alpha = 1.0 - fadeFactor;

        // Apply the calculated alpha to the outline color
        return fixed4(input.color.rgb, input.color.a * alpha);
      }
      ENDCG
    }
  }
}